...
from app.crud.studySession_crud import StudySessionCRUD
from app.models.studySession import StudySession, StudySessionCreate, StudySessionListResponse, StudySessionUpdate
...
@router.post("", dependencies=[Depends(has_permission("studySession:create"))], response_model=StudySession)
def create_item(
    item_in: StudySessionCreate,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
    current_dept_id: int = Depends(get_current_dept_id),
):
    logger.debug(f"Creating StudySession with current_dept_id: {current_dept_id}")
    logger.debug(f"Creating StudySession with current_user_id: {current_user_id}")
...
def list_items(
    ...
    if sortField and sortOrder:
        field = getattr(StudySession, sortField, None)
...
@router.get("/{item_id}", dependencies=[Depends(has_permission("studySession:show"))], response_model=StudySession)
def get_item(
    item_id: int, 
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
...
@router.patch("/{item_id}", dependencies=[Depends(has_permission("studySession:edit"))], response_model=StudySession)
def update_item(
    item_id: int,
    item_in: StudySessionUpdate,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
    current_dept_id: int = Depends(get_current_dept_id),
):
...
@router.delete("/{item_id}", dependencies=[Depends(has_permission("studySession:delete"))], response_model=StudySession)
def delete_item(
    item_id: int,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
