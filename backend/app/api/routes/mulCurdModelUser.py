...
from app.crud.mulCurdModelUser_crud import MulCurdModelUserCRUD
from app.models.mulCurdModelUser import MulCurdModelUser, MulCurdModelUserCreate, MulCurdModelUserListResponse, MulCurdModelUserUpdate
...
@router.post("", dependencies=[Depends(has_permission("super_admin"))], response_model=MulCurdModelUser)
def create_item(
    item_in: MulCurdModelUserCreate,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
):
    crud = MulCurdModelUserCRUD(session)
...
@router.get("", dependencies=[Depends(has_permission("super_admin"))], response_model=MulCurdModelUserListResponse)
def list_items(
    request: Request,
    _start: int = Query(0),
    _end: int = Query(10),
    ...
    if sortField and sortOrder:
        field = getattr(MulCurdModelUser, sortField, None)
...
@router.get("/{item_id}", dependencies=[Depends(has_permission("super_admin"))], response_model=MulCurdModelUser)
def get_item(item_id: int, session: Session = Depends(get_session)):
    crud = MulCurdModelUserCRUD(session)
    item = crud.get_by_id(item_id)
    if not item:
        raise HTTPException(status_code=404, detail="MulCurdModelUser not found")
    return item


@router.patch("/{item_id}", dependencies=[Depends(has_permission("super_admin"))], response_model=MulCurdModelUser)
def update_item(
    item_id: int,
    item_in: MulCurdModelUserUpdate,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
):
    crud = MulCurdModelUserCRUD(session)
...
@router.delete("/{item_id}", dependencies=[Depends(has_permission("super_admin"))], response_model=MulCurdModelUser)
def delete_item(
    item_id: int,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
