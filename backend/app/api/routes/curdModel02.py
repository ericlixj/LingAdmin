...
from app.crud.curdModel02_crud import CurdModel02CRUD
from app.models.curdModel02 import CurdModel02, CurdModel02Create, CurdModel02ListResponse, CurdModel02Update
...
@router.post("", dependencies=[Depends(has_permission("super_admin"))], response_model=CurdModel02)
def create_item(
    item_in: CurdModel02Create,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
):
    crud = CurdModel02CRUD(session)
    existing = crud.get_by_code(item_in.code)
    if existing:
        raise HTTPException(status_code=400, detail="CurdModel02 code already exists")
...
@router.get("", dependencies=[Depends(has_permission("super_admin"))], response_model=CurdModel02ListResponse)
def list_items(
    request: Request,
    _start: int = Query(0),
    _end: int = Query(10),
    ...
    if sortField and sortOrder:
        field = getattr(CurdModel02, sortField, None)
...
@router.get("/{item_id}", dependencies=[Depends(has_permission("super_admin"))], response_model=CurdModel02)
def get_item(item_id: int, session: Session = Depends(get_session)):
    crud = CurdModel02CRUD(session)
    item = crud.get_by_id(item_id)
    if not item:
        raise HTTPException(status_code=404, detail="CurdModel02 not found")
    return item


@router.patch("/{item_id}", dependencies=[Depends(has_permission("super_admin"))], response_model=CurdModel02)
def update_item(
    item_id: int,
    item_in: CurdModel02Update,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
):
    crud = CurdModel02CRUD(session)
...
@router.delete("/{item_id}", dependencies=[Depends(has_permission("super_admin"))], response_model=CurdModel02)
def delete_item(
    item_id: int,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
