...
from app.models.dept import Dept, DeptCreate, DeptListResponse, DeptUpdate
...
@router.post("", dependencies=[Depends(has_permission("dept:create"))], response_model=Dept)
def create_item(
    item_in: DeptCreate,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
):
    crud = DeptCRUD(session)
    item_in.creator = str(current_user_id)
    return crud.create(item_in)    

@router.get("", dependencies=[Depends(has_permission("dept:list"))], response_model=DeptListResponse)
def list_items(
    request: Request,
    _start: int = Query(0),
    _end: int = Query(10),
    ...
    if sortField and sortOrder:
        field = getattr(Dept, sortField, None)
...
@router.get("/{item_id}", dependencies=[Depends(has_permission("dept:get"))], response_model=Dept)
...
@router.patch("/{item_id}", dependencies=[Depends(has_permission("dept:update"))], response_model=Dept)
def update_item(
    item_id: int,
    item_in: DeptUpdate,
    session: Session = Depends(get_session),
...
@router.delete("/{item_id}", dependencies=[Depends(has_permission("dept:delete"))], response_model=Dept)
def delete_item(
    item_id: int,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
