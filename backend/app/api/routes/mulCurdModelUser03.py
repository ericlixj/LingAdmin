...
from app.crud.mulCurdModelUser03_crud import MulCurdModelUser03CRUD
from app.models.mulCurdModelUser03 import MulCurdModelUser03, MulCurdModelUser03Create, MulCurdModelUser03ListResponse, MulCurdModelUser03Update
...
@router.post("", dependencies=[Depends(has_permission("super_admin"))], response_model=MulCurdModelUser03)
def create_item(
    item_in: MulCurdModelUser03Create,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
):
    crud = MulCurdModelUser03CRUD(session)
...
@router.get("", dependencies=[Depends(has_permission("super_admin"))], response_model=MulCurdModelUser03ListResponse)
def list_items(
    request: Request,
    _start: int = Query(0),
    _end: int = Query(10),
    ...
    if sortField and sortOrder:
        field = getattr(MulCurdModelUser03, sortField, None)
...
@router.get("/{item_id}", dependencies=[Depends(has_permission("super_admin"))], response_model=MulCurdModelUser03)
def get_item(item_id: int, session: Session = Depends(get_session)):
    crud = MulCurdModelUser03CRUD(session)
    item = crud.get_by_id(item_id)
    if not item:
        raise HTTPException(status_code=404, detail="MulCurdModelUser03 not found")
    return item


@router.patch("/{item_id}", dependencies=[Depends(has_permission("super_admin"))], response_model=MulCurdModelUser03)
def update_item(
    item_id: int,
    item_in: MulCurdModelUser03Update,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
):
    crud = MulCurdModelUser03CRUD(session)
...
@router.delete("/{item_id}", dependencies=[Depends(has_permission("super_admin"))], response_model=MulCurdModelUser03)
def delete_item(
    item_id: int,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
