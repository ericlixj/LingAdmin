...
from app.crud.mulCurdModelOrder_crud import MulCurdModelOrderCRUD
from app.models.mulCurdModelOrder import MulCurdModelOrder, MulCurdModelOrderCreate, MulCurdModelOrderListResponse, MulCurdModelOrderUpdate
...
@router.post("", dependencies=[Depends(has_permission("super_admin"))], response_model=MulCurdModelOrder)
def create_item(
    item_in: MulCurdModelOrderCreate,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
):
    crud = MulCurdModelOrderCRUD(session)
...
@router.get("", dependencies=[Depends(has_permission("super_admin"))], response_model=MulCurdModelOrderListResponse)
def list_items(
    request: Request,
    _start: int = Query(0),
    _end: int = Query(10),
    ...
    if sortField and sortOrder:
        field = getattr(MulCurdModelOrder, sortField, None)
...
@router.get("/{item_id}", dependencies=[Depends(has_permission("super_admin"))], response_model=MulCurdModelOrder)
def get_item(item_id: int, session: Session = Depends(get_session)):
    crud = MulCurdModelOrderCRUD(session)
    item = crud.get_by_id(item_id)
    if not item:
        raise HTTPException(status_code=404, detail="MulCurdModelOrder not found")
    return item


@router.patch("/{item_id}", dependencies=[Depends(has_permission("super_admin"))], response_model=MulCurdModelOrder)
def update_item(
    item_id: int,
    item_in: MulCurdModelOrderUpdate,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
):
    crud = MulCurdModelOrderCRUD(session)
...
@router.delete("/{item_id}", dependencies=[Depends(has_permission("super_admin"))], response_model=MulCurdModelOrder)
def delete_item(
    item_id: int,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
