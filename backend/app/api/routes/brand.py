...
from app.models.brand import Brand, BrandCreate, BrandListResponse, BrandUpdate
...
@router.post("", dependencies=[Depends(has_permission("brand:create"))], response_model=Brand)
def create_item(
    item_in: BrandCreate,
    ...
    logger.debug(f"Creating Brand with current_dept_id: {current_dept_id}")
    logger.debug(f"Creating Brand with current_user_id: {current_user_id}")
    crud = BrandCRUD(session, user_id=current_user_id, dept_id=current_dept_id)
    item_in.creator = str(current_user_id)
    item_in.dept_id = current_dept_id
    return crud.create(item_in)

@router.get("", dependencies=[Depends(has_permission("brand:list"))], response_model=BrandListResponse)
def list_items(
    request: Request,
    _start: int = Query(0),
    _end: int = Query(10),
    ...
    if sortField and sortOrder:
        field = getattr(Brand, sortField, None)
...
@router.get("/{item_id}", dependencies=[Depends(has_permission("brand:show"))], response_model=Brand)
def get_item(
    item_id: int, 
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
...
@router.patch("/{item_id}", dependencies=[Depends(has_permission("brand:edit"))], response_model=Brand)
def update_item(
    item_id: int,
    item_in: BrandUpdate,
    session: Session = Depends(get_session),
...
@router.delete("/{item_id}", dependencies=[Depends(has_permission("brand:delete"))], response_model=Brand)
def delete_item(
    item_id: int,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
