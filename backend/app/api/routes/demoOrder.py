...
from app.crud.demoOrder_crud import DemoOrderCRUD
from app.models.demoOrder import DemoOrder, DemoOrderCreate, DemoOrderListResponse, DemoOrderUpdate
...
@router.post("", dependencies=[Depends(has_permission("super_admin"))], response_model=DemoOrder)
def create_item(
    item_in: DemoOrderCreate,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
):
    crud = DemoOrderCRUD(session)
...
@router.get("", dependencies=[Depends(has_permission("super_admin"))], response_model=DemoOrderListResponse)
def list_items(
    request: Request,
    _start: int = Query(0),
    _end: int = Query(10),
    ...
    if sortField and sortOrder:
        field = getattr(DemoOrder, sortField, None)
...
@router.get("/{item_id}", dependencies=[Depends(has_permission("super_admin"))], response_model=DemoOrder)
def get_item(item_id: int, session: Session = Depends(get_session)):
    crud = DemoOrderCRUD(session)
    item = crud.get_by_id(item_id)
    if not item:
        raise HTTPException(status_code=404, detail="DemoOrder not found")
    return item


@router.patch("/{item_id}", dependencies=[Depends(has_permission("super_admin"))], response_model=DemoOrder)
def update_item(
    item_id: int,
    item_in: DemoOrderUpdate,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
):
    crud = DemoOrderCRUD(session)
...
@router.delete("/{item_id}", dependencies=[Depends(has_permission("super_admin"))], response_model=DemoOrder)
def delete_item(
    item_id: int,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
