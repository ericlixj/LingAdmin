...
from app.crud.sysDicData001_crud import SysDicData001CRUD
from app.models.sysDicData001 import SysDicData001, SysDicData001Create, SysDicData001ListResponse, SysDicData001Update
...
@router.post("", dependencies=[Depends(has_permission("sysDicData001:create"))], response_model=SysDicData001)
def create_item(
    item_in: SysDicData001Create,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
):
    crud = SysDicData001CRUD(session)
    item_in.creator = str(current_user_id)
    return crud.create(item_in)    

@router.get("", dependencies=[Depends(has_permission("sysDicData001:list"))], response_model=SysDicData001ListResponse)
def list_items(
    request: Request,
    _start: int = Query(0),
    _end: int = Query(10),
    ...
    if sortField and sortOrder:
        field = getattr(SysDicData001, sortField, None)
...
@router.get("/{item_id}", dependencies=[Depends(has_permission("sysDicData001:get"))], response_model=SysDicData001)
def get_item(item_id: int, session: Session = Depends(get_session)):
    crud = SysDicData001CRUD(session)
    item = crud.get_by_id(item_id)
    if not item:
        raise HTTPException(status_code=404, detail="SysDicData001 not found")
    return item


@router.patch("/{item_id}", dependencies=[Depends(has_permission("sysDicData001:update"))], response_model=SysDicData001)
def update_item(
    item_id: int,
    item_in: SysDicData001Update,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
):
    crud = SysDicData001CRUD(session)
...
@router.delete("/{item_id}", dependencies=[Depends(has_permission("sysDicData001:delete"))], response_model=SysDicData001)
def delete_item(
    item_id: int,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
