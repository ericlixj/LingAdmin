...
from app.crud.sysDic001_crud import SysDic001CRUD
from app.models.sysDic001 import SysDic001, SysDic001Create, SysDic001ListResponse, SysDic001Update
...
@router.post("", dependencies=[Depends(has_permission("sysDic001:create"))], response_model=SysDic001)
def create_item(
    item_in: SysDic001Create,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
):
    crud = SysDic001CRUD(session)
    item_in.creator = str(current_user_id)
    return crud.create(item_in)    

@router.get("", dependencies=[Depends(has_permission("sysDic001:list"))], response_model=SysDic001ListResponse)
def list_items(
    request: Request,
    _start: int = Query(0),
    _end: int = Query(10),
    ...
    if sortField and sortOrder:
        field = getattr(SysDic001, sortField, None)
...
@router.get("/{item_id}", dependencies=[Depends(has_permission("sysDic001:get"))], response_model=SysDic001)
def get_item(item_id: int, session: Session = Depends(get_session)):
    crud = SysDic001CRUD(session)
    item = crud.get_by_id(item_id)
    if not item:
        raise HTTPException(status_code=404, detail="SysDic001 not found")
    return item


@router.patch("/{item_id}", dependencies=[Depends(has_permission("sysDic001:update"))], response_model=SysDic001)
def update_item(
    item_id: int,
    item_in: SysDic001Update,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
):
    crud = SysDic001CRUD(session)
...
@router.delete("/{item_id}", dependencies=[Depends(has_permission("sysDic001:delete"))], response_model=SysDic001)
def delete_item(
    item_id: int,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
