...
from app.crud.demoItems_crud import DemoItemsCRUD
from app.models.demoItems import DemoItems, DemoItemsCreate, DemoItemsListResponse, DemoItemsUpdate
...
@router.post("", dependencies=[Depends(has_permission("super_admin"))], response_model=DemoItems)
def create_item(
    item_in: DemoItemsCreate,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
):
    crud = DemoItemsCRUD(session)
...
@router.get("", dependencies=[Depends(has_permission("super_admin"))], response_model=DemoItemsListResponse)
def list_items(
    request: Request,
    _start: int = Query(0),
    _end: int = Query(10),
    ...
    if sortField and sortOrder:
        field = getattr(DemoItems, sortField, None)
...
@router.get("/{item_id}", dependencies=[Depends(has_permission("super_admin"))], response_model=DemoItems)
def get_item(item_id: int, session: Session = Depends(get_session)):
    crud = DemoItemsCRUD(session)
    item = crud.get_by_id(item_id)
    if not item:
        raise HTTPException(status_code=404, detail="DemoItems not found")
    return item


@router.patch("/{item_id}", dependencies=[Depends(has_permission("super_admin"))], response_model=DemoItems)
def update_item(
    item_id: int,
    item_in: DemoItemsUpdate,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
):
    crud = DemoItemsCRUD(session)
...
@router.delete("/{item_id}", dependencies=[Depends(has_permission("super_admin"))], response_model=DemoItems)
def delete_item(
    item_id: int,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
