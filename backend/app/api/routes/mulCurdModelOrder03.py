...
from app.crud.mulCurdModelOrder03_crud import MulCurdModelOrder03CRUD
from app.models.mulCurdModelOrder03 import MulCurdModelOrder03, MulCurdModelOrder03Create, MulCurdModelOrder03ListResponse, MulCurdModelOrder03Update
...
@router.post("", dependencies=[Depends(has_permission("super_admin"))], response_model=MulCurdModelOrder03)
def create_item(
    item_in: MulCurdModelOrder03Create,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
):
    crud = MulCurdModelOrder03CRUD(session)
...
@router.get("", dependencies=[Depends(has_permission("super_admin"))], response_model=MulCurdModelOrder03ListResponse)
def list_items(
    request: Request,
    _start: int = Query(0),
    _end: int = Query(10),
    ...
    if sortField and sortOrder:
        field = getattr(MulCurdModelOrder03, sortField, None)
...
@router.get("/{item_id}", dependencies=[Depends(has_permission("super_admin"))], response_model=MulCurdModelOrder03)
def get_item(item_id: int, session: Session = Depends(get_session)):
    crud = MulCurdModelOrder03CRUD(session)
    item = crud.get_by_id(item_id)
    if not item:
        raise HTTPException(status_code=404, detail="MulCurdModelOrder03 not found")
    return item


@router.patch("/{item_id}", dependencies=[Depends(has_permission("super_admin"))], response_model=MulCurdModelOrder03)
def update_item(
    item_id: int,
    item_in: MulCurdModelOrder03Update,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
):
    crud = MulCurdModelOrder03CRUD(session)
...
@router.delete("/{item_id}", dependencies=[Depends(has_permission("super_admin"))], response_model=MulCurdModelOrder03)
def delete_item(
    item_id: int,
    session: Session = Depends(get_session),
    current_user_id: int = Depends(get_current_user_id),
