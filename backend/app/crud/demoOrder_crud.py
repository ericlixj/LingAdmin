...
from app.models.demoOrder import DemoOrder, DemoOrderCreate, DemoOrderUpdate
...
class DemoOrderCRUD:
    def __init__(self, session: Session):
        ...
        self.model = DemoOrder

    def get_by_id(self, demoOrder_id: int) -> Optional[DemoOrder]:
        statement = select(DemoOrder).where(DemoOrder.id == demoOrder_id, DemoOrder.deleted == False)
        result = self.session.exec(statement).first()
        return result

    def create(self, obj_in: DemoOrderCreate) -> DemoOrder:
        db_obj = DemoOrder.from_orm(obj_in)
        self.session.add(db_obj)
        self.session.commit()
        self.session.refresh(db_obj)
        return db_obj

    def update(self, db_obj: DemoOrder, obj_in: DemoOrderUpdate) -> DemoOrder:
    ...
    def soft_delete(self, db_obj: DemoOrder) -> DemoOrder:
        db_obj.deleted = True
        db_obj.update_time = datetime.utcnow()
        self.session.add(db_obj)
        self.session.commit()
    ...
    def list_all(
        ...
        query = select(DemoOrder).where(DemoOrder.deleted == False)
        query = self._apply_filters(query, filters)
        if order_by is not None:
            query = query.order_by(order_by)
        else:
            query = query.order_by(DemoOrder.id.desc())
        logger.debug(f"Executing query: {query}")
        return self.session.exec(query.offset(skip).limit(limit)).all()

    def count_all(self, filters: Optional[Dict[str, Any]] = None) -> int:
        query = select(func.count()).select_from(DemoOrder).where(DemoOrder.deleted == False)
        query = self._apply_filters(query, filters)
