...
from app.models.sysDic001 import SysDic001, SysDic001Create, SysDic001Update
from app.models.sysDicData001 import SysDicData001
...
class SysDic001CRUD:
    def __init__(self, session: Session):
        ...
        self.model = SysDic001

    def get_by_id(self, sysDic001_id: int) -> Optional[SysDic001]:
        statement = select(SysDic001).where(SysDic001.id == sysDic001_id, SysDic001.deleted == False)
        result = self.session.exec(statement).first()
        return result

    def create(self, obj_in: SysDic001Create) -> SysDic001:
        db_obj = SysDic001(**obj_in.dict())
        self.session.add(db_obj)
        self.session.commit()
        self.session.refresh(db_obj)
        return db_obj

    def update(self, db_obj: SysDic001, obj_in: SysDic001Update) -> SysDic001:
    ...
    def soft_delete(self, db_obj: SysDic001) -> SysDic001:
        db_obj.deleted = True
        db_obj.update_time = datetime.utcnow()
        self.session.add(db_obj)
        self.session.execute(
            update(SysDicData001)
            .where(SysDicData001.sys_dic_id == db_obj.id)
            .values(
                deleted=True,
    ...
    def list_all(
        ...
        query = select(SysDic001).where(SysDic001.deleted == False)
        query = self._apply_filters(query, filters)
        if order_by is not None:
            query = query.order_by(order_by)
        else:
            query = query.order_by(SysDic001.id.desc())
        logger.debug(f"Executing query: {query}")
        return self.session.exec(query.offset(skip).limit(limit)).all()

    def count_all(self, filters: Optional[Dict[str, Any]] = None) -> int:
        query = select(func.count()).select_from(SysDic001).where(SysDic001.deleted == False)
        query = self._apply_filters(query, filters)
