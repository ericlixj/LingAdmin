...
from app.models.sysDic import SysDic, SysDicCreate, SysDicUpdate
from app.models.sysDicData import SysDicData
...
class SysDicCRUD:
    ...
    def get_by_id(self, sysDic_id: int) -> Optional[SysDic]:
        statement = select(SysDic).where(SysDic.id == sysDic_id, SysDic.deleted == False)
        result = self.session.exec(statement).first()
        return result

    def create(self, obj_in: SysDicCreate) -> SysDic:
        db_obj = SysDic(**obj_in.dict())
        self.session.add(db_obj)
        self.session.commit()
        self.session.refresh(db_obj)
        return db_obj

    def update(self, db_obj: SysDic, obj_in: SysDicUpdate) -> SysDic:
    ...
    def soft_delete(self, db_obj: SysDic) -> SysDic:
        db_obj.deleted = True
        db_obj.update_time = datetime.utcnow()
        self.session.add(db_obj)
        self.session.execute(
            update(SysDicData)
            .where(SysDicData.sys_dic_id == db_obj.id)
            .values(
                deleted=True,
    ...
    def list_all(
        ...
        query = select(SysDic).where(SysDic.deleted == False)
        query = self._apply_filters(query, filters)
        if order_by is not None:
            query = query.order_by(order_by)
        else:
            query = query.order_by(SysDic.id.desc())
        logger.debug(f"Executing query: {query}")
        return self.session.exec(query.offset(skip).limit(limit)).all()

    def count_all(self, filters: Optional[Dict[str, Any]] = None) -> int:
        query = select(func.count()).select_from(SysDic).where(SysDic.deleted == False)
        query = self._apply_filters(query, filters)
