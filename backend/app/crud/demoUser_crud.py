...
from app.models.demoUser import DemoUser, DemoUserCreate, DemoUserUpdate
...
class DemoUserCRUD:
    ...
    def get_by_id(self, demoUser_id: int) -> Optional[DemoUser]:
        statement = select(DemoUser).where(DemoUser.id == demoUser_id, DemoUser.deleted == False)
        result = self.session.exec(statement).first()
        return result

    def create(self, obj_in: DemoUserCreate) -> DemoUser:
        db_obj = DemoUser(**obj_in.dict())
        self.session.add(db_obj)
        self.session.commit()
        self.session.refresh(db_obj)
        return db_obj

    def update(self, db_obj: DemoUser, obj_in: DemoUserUpdate) -> DemoUser:
    ...
    def soft_delete(self, db_obj: DemoUser) -> DemoUser:
        db_obj.deleted = True
        db_obj.update_time = datetime.utcnow()
        self.session.add(db_obj)
        self.session.commit()
    ...
    def list_all(
        ...
        query = select(DemoUser).where(DemoUser.deleted == False)
        query = self._apply_filters(query, filters)
        if order_by is not None:
            query = query.order_by(order_by)
        else:
            query = query.order_by(DemoUser.id.desc())
        logger.debug(f"Executing query: {query}")
        return self.session.exec(query.offset(skip).limit(limit)).all()

    def count_all(self, filters: Optional[Dict[str, Any]] = None) -> int:
        query = select(func.count()).select_from(DemoUser).where(DemoUser.deleted == False)
        query = self._apply_filters(query, filters)
