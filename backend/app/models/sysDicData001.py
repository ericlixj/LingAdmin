...
class SysDicData001(SQLModel, table=True):
    __tablename__ = "sys_dic_data001"
    id:        int = Field(
        description="主键",
        sa_column=Column(
            Integer,
...
class SysDicData001Create(SQLModel):
    label: str
    value: str
    status: int
    orderby: int
...
class SysDicData001Update(SQLModel):
    sys_dic_id: Optional[int] = None
...
class SysDicData001ListResponse(SQLModel):
    data: List[SysDicData001]
    total: int