...
class MulCurdModelUser(SQLModel, table=True):

    id: int = Field(default=None, primary_key=True, description="主键")

    user_name: str = Field(default="", max_length=100, index=True, unique=True, description="用户名称")
...
class MulCurdModelUserCreate(SQLModel):
    user_name: str
    user_code: str
...
class MulCurdModelUserUpdate(SQLModel):
...
class MulCurdModelUserListResponse(SQLModel):
    data: List[MulCurdModelUser]
    total: int