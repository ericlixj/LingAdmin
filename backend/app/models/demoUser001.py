...
class DemoUser001(SQLModel, table=True):
    __tablename__ = "demo_user001"
    id:        int = Field(
        description="主键",
        sa_column=Column(
            Integer,
...
class DemoUser001Create(SQLModel):
    name: str
    creator: Optional[str] = Field(default=None, max_length=64)

class DemoUser001Update(SQLModel):
    name: Optional[str] = None
    updater: Optional[str] = Field(default=None, max_length=64)

class DemoUser001ListResponse(SQLModel):
    data: List[DemoUser001]
    total: int