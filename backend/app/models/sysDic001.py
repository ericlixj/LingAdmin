...
class SysDic001(SQLModel, table=True):
    __tablename__ = "sys_dic001"
    id:        int = Field(
        description="主键",
        sa_column=Column(
            Integer,
...
class SysDic001Create(SQLModel):
    dic_code: str
    dic_name: str
    status: int
...
class SysDic001Update(SQLModel):
...
class SysDic001ListResponse(SQLModel):
    data: List[SysDic001]
    total: int