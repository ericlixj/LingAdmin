...
class FsaFlyerLink(SQLModel, table=True):
    __tablename__ = "fsa_flyer_link"
    id:        int = Field(
        description="主键",
        sa_column=Column(
            Integer,
...
class FsaFlyerLinkCreate(SQLModel):
    flyer_id: Optional[int] = None
    fsa: Optional[str] = None
    creator: Optional[str] = Field(default=None, max_length=64)
    dept_id: Optional[int] = None

class FsaFlyerLinkUpdate(SQLModel):
    flyer_id: Optional[int] = None
    fsa: Optional[str] = None
    updater: Optional[str] = Field(default=None, max_length=64)
    dept_id: Optional[int] = None

class FsaFlyerLinkListResponse(SQLModel):
    data: List[FsaFlyerLink]
    total: int