...
from codegen.generator.backend_generator import generate_backend_files
from codegen.generator.frontend_generator import generate_frontend_files
from codegen.utils.json_loader import load_single_module_json

def process_model(model: CURDModel, target_dir: str = "target/single_module") -> dict:
...
def process_model_in_memory(model: CURDModel) -> dict:
...
def process_module_from_dict(model_data: dict) -> dict:
    model = CURDModel.model_validate(model_data)
